...
def visualize(labels, results, img):
    ...
    for result in results:
        ...
        for box_data in box_datas:
            ...
            label = labels[int(id_label)].upper()
...
def main():
    model_path = "best.pt"
    video_path = r"C:\Users\user\Desktop\Training OCR\Example 3 Data\Video3.mp4"
    ...
    while cap.isOpened():
        ...
        results = trained_model.predict(source=frame, verbose=False, conf=0.5)
        
        visualize(labels, results, frame)
        cropped_images = code_image(results, frame)
        
        for cropped_image in cropped_images:
            clean_image = preprocessing(cropped_image)
